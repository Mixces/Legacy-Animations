...
public class ItemUtils {
    ...
    public boolean isValidItem(ItemStack heldStack, UseAction action) {
        if (ItemUtils.INSTANCE.isValidHeldItem()) {
            ...
            if (isEating && foodItem.canAlwaysEat()) return true;
            else if (!isEating) return action != UseAction.NONE;
        }
        return false;
    }
     ...
     *
     * @param stack        We need the currently held item!
     * @return Returns whether the main hand item is a rod, tool, trident or item along those lines.
     */
    public boolean isFull3d(ItemStack stack) {
        final ClientPlayerEntity player = MinecraftClient.getInstance().player;
        if (player == null) return false;
        final Item item = stack == null ? player.getMainHandStack().getItem() : stack.getItem();
        if (item == null) return false;
        return shouldRotateAroundWhenRendering(stack, true) || item instanceof ToolItem || item instanceof TridentItem;
    }
    ...
    public boolean isUsing(ClientPlayerEntity player) {
        return ServerUtils.INSTANCE.isValidServer() ? MinecraftClient.getInstance().options.useKey.isPressed() : player.isUsingItem();
    }
}
